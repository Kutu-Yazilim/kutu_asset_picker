...
///
/// **The viewport is stage-sized, not window-sized.** It fills the space it is
/// given and centres the window inside it, and the clip sits at the stage's
/// edge rather than the window's. That is what lets the rest of the media show
/// through the dimming mask beyond the window instead of being cut away to a
/// flat band of background, and what lets a drag that starts on that dimmed
/// footage still pan. The crop math is unchanged and stays in window
/// coordinates: the one conversion is the gesture's focal point, rebased from
/// the stage onto the window before it meets `focalAnchoredOffset`.
///
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final stage = cropStageSize(constraints, window);
        // Gestures arrive in stage coordinates; the crop math works in window
        // coordinates. The window is centred in the stage, so the rebase is
        // its top-left corner.
        final windowOrigin = cropWindowRect(stage, window).topLeft;
        return ClipRect(
          child: SizedBox.fromSize(
            ...
            child: GestureDetector(
              ...
              onScaleStart: (details) {
                ...
                startFocal.value = details.localFocalPoint - windowOrigin;
              },
                  ...
                  state.copyWith(
                    ...
                    offset: clampOffset(
                      focalAnchoredOffset(
                        ...
                        focal: details.localFocalPoint - windowOrigin,
              ...
              onScaleEnd: (details) {
                ...
                if (velocity.distance < AssetPickerLimits.minFlingVelocity) {
                  return;
                }
                final seconds = flingDuration(velocity.distance);
                if (seconds <= 0) return;

                // A clamped ballistic run: the simulation proposes a
                // destination, and every frame is pushed back through
                // `clampOffset`, so the image decelerates naturally and still
                // cannot leave a gap.
